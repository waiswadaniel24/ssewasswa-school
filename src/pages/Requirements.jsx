...
export default function Requirements() {
  ...
  var fetchData = async function() {
    try {
      ...
      if (rRes.success && rRes.data.length > 0) { setRequirements(JSON.parse(rRes.data[0].value)); }
      var recRes = await window.electronAPI.queryDatabase("SELECT student_id, requirement_name, submitted FROM student_requirements");
      if (recRes.success) {
        var map = {};
        recRes.data.forEach(function(r) { if(!map[r.student_id]) map[r.student_id] = {}; map[r.student_id][r.requirement_name] = r.submitted; });
        setRecords(map);
      }
  ...
  var addRequirement = async function() {
    ...
    setNewReq({name:'', section:'All'}); setMsg('Requirement added!');
  };
  ...
  var updateRecord = async function(studentId, reqName, value) {
    ...
    setRecords(function(prev) { var next = {...prev}; if(!next[studentId]) next[studentId] = {}; next[studentId][reqName] = value; return next; });
  };
  var printForm = function() {
    var doc = new jsPDF();
    doc.setFontSize(18); doc.text("School Requirements & Fees Form", 105, 20, { align: 'center' });
    doc.setFontSize(12); doc.text("Date: " + new Date().toLocaleDateString(), 14, 30);
    var y = 40; doc.setFontSize(14); doc.text("Requirements Checklist:", 14, y); y += 8; doc.setFontSize(11);
    requirements.forEach(function(r, i) {
      if (y > 270) { doc.addPage(); y = 20; }
      doc.text(i+1 + ". " + r.name + " (" + r.section + ")", 16, y); doc.rect(150, y-4, 5, 5); y += 8;
    });
    y += 10; if (y > 250) { doc.addPage(); y = 20; }
    doc.setFontSize(14); doc.text("Parent/Guardian Declaration:", 14, y); y += 8; doc.setFontSize(11);
    doc.text("I confirm that the above requirements have been provided:", 14, y); y += 15; doc.text("Signature: ______________________", 14, y);
      ...
      <div className="card">
        ...
        <div className="card-body" style={{ overflowX: 'auto' }}>
          <table className="data-table">
            <thead>
              <tr><th>Adm No</th><th>Student Name</th><th>Section</th>{requirements.map(function(r, i) { return <th key={i} style={{ textAlign: 'center', minWidth: '120px' }}>{r.name}</th>; })}</tr>
            </thead>
            <tbody>
              {students.map(function(s) {
                return (
                  <tr key={s.id}>
                    <td>{s.admission_number || '-'}</td><td>{s.first_name} {s.last_name}</td><td>{s.student_type || 'Day'}</td>
                    {requirements.map(function(r, i) {
                      ...
                      return (
                        <td key={i} style={{ textAlign: 'center' }}>
                          {applies ? (
                            <input type="text" value={val === 1 ? '' : val} placeholder="e.g. 6/12 or Done" onChange={function(e) { updateRecord(s.id, r.name, e.target.value); }} style={{ width: '90px', padding: '4px', border: '1px solid #ccc', borderRadius: '4px', textAlign: 'center' }} />
