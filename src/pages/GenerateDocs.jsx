...
export default function GenerateDocs() {
  ...
  const loadAll = useCallback(async function () {
    ...
    try {
      ...
      var schoolData = (sRes && sRes.success && sRes.data && sRes.data[0]) ? { ...sRes.data[0] } : {};
      ...
      try {
        ...
        if (!mountedRef.current) return;
        if (sig && sig.success) setSignatures(sig.data || {});
          ...
          <div className="card">
            ...
            <div className="card-body">
              <p style={{ marginBottom: '15px', color: '#666', fontSize: '14px' }}>
                Automatically generates a printable PDF containing the schools requirement list and fee structure for parents.
              </p>
              <button
                className="btn btn-primary"
                onClick={async function () {
                  ...
                  if (!window.jspdf || !window.jspdf.jsPDF) {
                    showMessage('❌ PDF library (jsPDF) not loaded');
                    return;
                  }
                  try {
